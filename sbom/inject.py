...
import sbom.elfcrypto as self_c
...
def scan_image(
    ...
    elf_inference = self_c.infer_from_elf(
        image_reference=str(image_ref),
        oci_client=oci_client,
    )
    if elf_inference:
        elf_cbom_bytes = self_c.build_inferred_cbom(
            image_ref=image_ref,
        ...
        scbom.push_cbom_referrer(
            ...
            image_reference=image_ref,
            oci_client=oci_client,
            tool_version=self_c.TOOL_NAME,
            extra_annotations={self_c.ANALYSIS_METHOD_ANNOTATION: self_c.ANALYSIS_METHOD_VALUE},
        )
        logger.info(
