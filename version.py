...
def iter_upgrade_path(
    ...
    versions = [ # <parsed, original>
        (pv, v) for v in versions if (pv := parse_to_semver(v)) > whence and pv <= whither
    ]
    versions = sorted(
        versions,
        key=lambda x: x[0],
